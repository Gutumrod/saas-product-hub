# CM01 — Product Gate Candidate C

_(anonymized candidate; identity withheld from synthesizer)_

---

# CM01 — Booking Claim & Case Management Module — C Expert Answer
...
**Role:** Independent expert . No gate verdict, no pricing/licensing decision, no code/docs mutation.

---

## Recommendation

CM01 is a **local-first, Thai-first React case-management UI template/module** (a `one_time_source_product`), not a deployable product and not a feature of BK01. It should be sold as a **UI template license to frontend developers / web agencies** who embed it in their own client work, with the **V1 finish line being a sellable $39 single-use template tier** — the code is already complete and CI-green, so the remaining distance to V1 is packaging + hardening, **not** a backend adapter.
...
- **For whom:** the **buyer** is a frontend dev / web agency licensing the UI to embed in their own client work; the **end user** is a single-role case officer (no auth by design, PRD §2).
- **Where V1 ends:** a sellable template tier. Backend capability is **not required for usable V1** (template tier); it is required only for the higher $129 agency tier and the ฿350/mo hosted-widget option (both post-adapter).
...
- `gh run list` shows 3 recent **success** runs — `33671032357` (push main @ 6202108), `33670789635` (push main @ aeaa750), `33670273164` (PR #1) — plus one superseded **failure** `33128547044` (ff15819, the timezone-harness defect). Matches `docs/CURRENT_STATUS.md`.
- Local rerun on this host: `npm test` = **63/63 passed** (13 files); `npm run typecheck` = pass; `npm run build` = pass (dist JS 199.24 kB / gzip 61.86 kB). E2E (28 Chromium tests) is documented as passing locally but is **not** in CI.
...
- `docs/CURRENT_STATUS.md`: CI green on main; lint disabled (P0b, master plan CM-D); `npm audit` advisory-only (1 moderate / 3 high / 2 critical, all dev toolchain).
...
- `docs/daily/2026-09-03.md`: P0a-C1 reassessed PASS after the CM01 timezone fix; no pre-existing assertion changed.
...
3. **Backend is not a V1 gate for the template tier.** The localStorage-only/no-auth design is *by design* (PRD §2, §8) and the README non-claims are honest. A $39 template can be sold as a local-first boilerplate with a clear "BYO backend, repository contract ready" scope. The backend adapter is the gate for the $129 agency tier and the ฿350/mo hosted widget, not for the $39 template tier.
...
- CI-green status is accepted as verified (confirmed via `gh run list` and a local rerun of test/typecheck/build on this host).

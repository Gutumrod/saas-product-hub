-- lane-b-role-lane_b_rw_live-teardown.sql
...
REVOKE SELECT, INSERT, DELETE ON wstera_platform_internal.runtime_token_grants FROM lane_b_rw_live;
REVOKE USAGE ON SCHEMA wstera_platform_internal FROM lane_b_rw_live;

-- 2. REVOKE ownership membership where granted (L6)
REVOKE ps01_migrator FROM lane_b_rw_live;

-- 3. terminate the role's sessions
SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE usename = 'lane_b_rw_live';

-- 4. drop the role (role-drop only; owner reassignment is not used — F9: refused by non-superuser postgres)
DROP ROLE lane_b_rw_live;

-- 5. closing assertion: L1 — catalog presence of the role itself is false
DO $lane_b_assert$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'lane_b_rw_live') THEN
    RAISE EXCEPTION 'L1: role lane_b_rw_live still present after DROP';
  END IF;
END
$lane_b_assert$;

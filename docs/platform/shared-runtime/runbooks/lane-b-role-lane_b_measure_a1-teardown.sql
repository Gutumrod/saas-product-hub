-- lane-b-role-lane_b_measure_a1-teardown.sql
...
REVOKE SELECT ON ps01.commercial_packages FROM lane_b_measure_a1;
REVOKE USAGE ON SCHEMA ps01_internal FROM lane_b_measure_a1;
REVOKE USAGE ON SCHEMA ps01 FROM lane_b_measure_a1;

-- 3. terminate the role's sessions
SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE usename = 'lane_b_measure_a1';

-- 4. drop the role (role-drop only; owner reassignment is not used — F9: refused by non-superuser postgres)
DROP ROLE lane_b_measure_a1;

-- 5. closing assertion: L1 — catalog presence of the role itself is false
DO $lane_b_assert$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'lane_b_measure_a1') THEN
    RAISE EXCEPTION 'L1: role lane_b_measure_a1 still present after DROP';
  END IF;
END
$lane_b_assert$;

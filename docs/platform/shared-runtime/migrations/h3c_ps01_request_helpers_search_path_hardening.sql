-- H3C PS01 request-helper search_path hardening — WSTERA LAB only.
-- Metadata-only remediation: pin three existing SECURITY INVOKER helpers to pg_catalog.
-- Source: BRIEF-H3C-PS01-HELPER-SEARCH-PATH-HARDENING-2026-09-09.md
...
BEGIN
  IF current_user <> 'postgres' THEN
    RAISE EXCEPTION 'H3C helper hardening requires platform postgres session';
  END IF;
  ...
  FOREACH v_name IN ARRAY ARRAY[
    ...
    IF v_config IS NOT NULL THEN
      RAISE EXCEPTION 'ps01.%() already has function-local config; refresh evidence before apply', v_name;
    END IF;
    EXECUTE format(
      'ALTER FUNCTION ps01.%I() SET search_path = pg_catalog',
      v_name
    );
    ...
    IF v_after_owner IS DISTINCT FROM v_owner
      ...
      RAISE EXCEPTION 'Unexpected metadata drift while hardening ps01.%()', v_name;
    END IF;
    IF v_after_config IS DISTINCT FROM ARRAY['search_path=pg_catalog']::text[] THEN

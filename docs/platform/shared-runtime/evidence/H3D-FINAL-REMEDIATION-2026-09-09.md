...
| `docs/platform/shared-runtime/fixtures/h3d-expected-catalog-manifest.json` | NEW — expected manifest. Prior live-captured fingerprint was `001c2c213f4a7086704da1e56879b82bee88f6f6054bd66c316e1791369d0619` (pre-H3D-S; 20 shop-rooted tables); the new fingerprint `3413b3494dfd3e1c264600c1cee5605f82c4c36ad8d9112156558a8202ecd806` was recomputed offline in H3D-S from the same captured content minus the two non-FK tables from `graph_tables` plus `monitored_non_fk_surfaces`; live `--verify` against the new manifest is PENDING (requires SELECT-only LAB credential). 18 shop-rooted FK tables, 2 monitored non-FK surfaces, 30 FK edges, 21 triggers. |
| `docs/platform/shared-runtime/fixtures/h3d-authz-fixture-seed.sql` | REWRITTEN per §5 — `SET LOCAL` bounded timeouts, `SHARE ROW EXCLUSIVE` on the 8 controlled tables (camera_access_audit added in H3D-S), `commercial_packages('starter') FOR SHARE` + semantic assertions, inline trigger gate, empty-baseline + case-normalised label + exact-id collision assertions, in-txn count snapshot, exact `+(2,2,2,1,1,2,2)` delta, every-other-graph-table `+0`, exact subscription semantics (`trial_ends_at = trial_started_at + 30d`, source `bootstrap`, no cancel/suspend, null period/grace) and exact audit semantics (`system` / `subscription.initialized` / `bootstrap` / expected reason / `previous_*` + `actor_id` + `idempotency_key` + `request_fingerprint` null), `SET CONSTRAINTS ALL IMMEDIATE`, redacted manifest with generated subscription/audit ids + `txid_current()` + pre-seed counts. No `ON CONFLICT`. |
...
- Catalog manifest fingerprint: prior live-captured fingerprint was `001c2c21…0619` (pre-H3D-S: 20 shop-rooted tables, 30 FK edges, 21 triggers; `subscription_audit_log <- shops` and `<- shop_subscriptions` both `ON DELETE CASCADE`; `commercial_packages('starter')` = room_limit 10 / pet_history_limit 300 ≫ the 1 room + 2 pets the fixture inserts). The new fingerprint `3413b349…` was recomputed offline in H3D-S from the same captured content minus the two non-FK tables from `graph_tables` plus `monitored_non_fk_surfaces` (18 shop-rooted FK tables, 2 monitored non-FK surfaces: `camera_access_audit`, `camera_rate_limit_buckets`, 30 FK edges, 21 triggers).
- `node tools/shared-runtime/h3d/catalog-manifest.mjs --verify <expected-manifest.json>`: pre-H3D-S run against the prior live-captured manifest (`001c2c21…0619`) showed **no drift**; live `--verify` against the new manifest (`3413b349…`) is **PENDING** (requires SELECT-only LAB credential).
...
2. concurrent INSERT/UPDATE/DELETE on each of the 8 controlled tables (camera_access_audit added in H3D-S) while the seed holds locks → contender blocks then `lock_timeout` on the contender; seed atomic.
...
| 2 | fresh SELECT-only catalog proof matches canonical shapes | **Met (pre-H3D-S)** — prior live-captured fingerprint was `001c2c21…0619` (`node tools/shared-runtime/h3d/catalog-manifest.mjs --verify <expected-manifest.json>` showed no drift pre-H3D-S); new fingerprint `3413b349…` was recomputed offline in H3D-S from the same captured content minus the two non-FK tables from `graph_tables` plus `monitored_non_fk_surfaces`; live `--verify` against the new manifest is **PENDING** (requires SELECT-only LAB credential); RPC SQLSTATE verified `P0001` |
...
| catalog verify | `node tools/shared-runtime/h3d/catalog-manifest.mjs --verify <expected-manifest.json>` | no drift (pre-H3D-S; live verify against new manifest PENDING) |

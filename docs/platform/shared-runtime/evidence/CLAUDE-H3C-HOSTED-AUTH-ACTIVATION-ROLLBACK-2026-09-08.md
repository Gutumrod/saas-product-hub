...
5. If the Dashboard rejects a non-`public` schema in the picker (unverified — Dashboard examples use `public`): **STOP**. Do not work around it by moving the function to `public`. Record the exact rejection text as a BLOCKER and hand back to House to decide (options: (a) file with Supabase support, (b) accept a dedicated non-exposed schema name the Dashboard does accept, still `REVOKE ALL FROM PUBLIC` and not in `pgrst.db_schemas`, (c) Management API PATCH as the only remaining path, accepting defect #36861 risk). Do not proceed to STEP 3 until this is resolved by House.
...
3. Run the proof harness `tools/shared-runtime/h3c/h3c-proof-harness.mjs` (WP-D). It authenticates the LAB service identity out-of-band, then verifies the decoded token (`iss`, `role=ps01_line_runtime`, `exp − iat ≤ 300`, signature against LAB JWKS) and runs the positive/negative matrix.
...
1. **Invalidate the service identity first.** Via a supported Auth Admin path: delete the LAB service user (preferred), or ban it and revoke all its sessions/refresh tokens. Confirm `auth.sessions` / `auth.refresh_tokens` for that `user_id` are gone. Record the UUID and the action in evidence.
...
|---|---|---|
| Dashboard / hosted validation accepts a `pg-functions://` URI in a **non-`public`** schema | **UNPROVEN** | STEP 2 outcome (accepted, or the exact rejection text) |

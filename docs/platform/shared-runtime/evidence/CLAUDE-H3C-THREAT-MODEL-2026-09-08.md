...
- **Residual already-issued token (House review H-07).** Deleting/banning the identity and clearing the grant row stops *new* tokens but does **not** invalidate an access token already in someone's hand: PostgREST validates the JWT signature + `exp` and does not check current Auth-user existence. A previously-minted `ps01_line_runtime` JWT stays usable (within its 3-RPC contract) until its signed `exp` — ≤ 5 minutes. A previously-minted `authenticated` JWT (if the identity had refreshed post-grant-removal, i.e. the wrong teardown order) stays usable until *its* `exp` (project default, up to 1 h). Teardown evidence must record the last issued token `exp` (`residualNarrowAuthorityUntil` in the harness output) and not declare authority gone before then (or before demonstrating that token is now rejected). Activation/rollback doc STEP 4.0 + STEP 4.7.
- **Blocks H3C?** **Y until the runbook is reordered.** The brief's current "Rollback Order" (disable hook → remove grant rows → … → identity revocation deferred to H3C-8 bullet 2) is the wrong sequence. `CLAUDE-H3C-HOSTED-AUTH-ACTIVATION-ROLLBACK-2026-09-08.md` STEP 4 encodes the fix (record residual `exp` → kill identity → clear grant → disable hook → SQL rollback → residual close-out). Once adopted: **N**.

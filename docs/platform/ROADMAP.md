...
> BK01 "Done" wording now reads "feature complete, not production safe"; §A1's CM01 2026-08-21
> removal is marked superseded by the 2026-08-27 seven-product lock; and §"Active scope"'s HC01 line
> now states the reference server and its tests exist only on open PR #1 (`feat/reference-server`),
...
|---|---|---|
| `booking` | Existing Project B baseline; public slug is `service-booking`, so the canonical customer-facing application host under the 2026-09-25 D1 policy is `service-booking.wstera.com`. The former `bk01.wstera.com` code-host reservation is superseded before launch. The root `wstera.com` remains the SaaS Product Hub's domain. Cloudflare Workers deploy is scaffolded (`@opennextjs/cloudflare`, Worker names `wstera-consumer`/`wstera-admin`) but not yet deployed or attached to the public host. | Phase 0 governs its baseline/security work; public launch also needs Stripe, a verified live deploy on `service-booking.wstera.com`, and live migration/release evidence. |
...
2. **`booking_ticket_module`** (`CM01`) — closest-to-done template in the portfolio (61/61 tests,
   ...
   tenants).
3. **`headless_commerce`** (`HC01`) — four modules exist on default branch `master`; a reference
   server exists only on open PR #1 (`feat/reference-server`), not merged, and reproducibly passes
   13/14 tests on the audit host (see line 185 below and master plan §2), not 14/14.
...
| `booking` | Most mature (28 migrations, real Stripe/auth/tenant code, DB-level hold/collision protection, real LINE HMAC). **2026-08-19 deep verification found quota/staff/top-up limits from `PRICING_SPEC.md` enforced nowhere in code — fixed same day** (`booking@ed06fa2`, migration `20260819000000_quota_staff_topup_enforcement.sql`, QA PASS=6/FAIL=0 in dev DB). Phase 0 baseline (§0 gate 2) **closed 2026-08-20** — see `SHARED_SAAS_RUNTIME_PROJECT_B_PLAN.md` §5 and `products/booking/docs/platform/PHASE_0_BASELINE_SNAPSHOT_2026-08-20.md`. **Corrected under P0a-B4:** this "Done" was feature-completion only — BK01 has no application test layer and currently fails clean lint/build (see `PORTFOLIO_PRODUCTION_MASTER_PLAN.md` §2). | Feature complete, not production safe |

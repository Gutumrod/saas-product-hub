...
-- Status: AUTHORIZED — applied to Project A under the bounded Owner authorization recorded in
--         docs/platform/house-long-run/R15-D0-DECISION-OUTCOME-2026-09-20.md §5
--         (Owner approval 2026-09-21 06:43 Asia/Bangkok, "อนุมัติตาม §5").
-- Purpose: create the objects that exist in drizzle/schema.ts but had no migration anywhere:
--          the two enum types and public.product_installations (with its FKs and unique index).
--          Verified column-by-column against drizzle/schema.ts:132-165, and dry-run verified in a
--          rolled-back transaction (BEGIN -> DDL -> assertions -> ROLLBACK) before applying.

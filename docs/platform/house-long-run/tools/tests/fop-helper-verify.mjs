#!/usr/bin/env node
/**
 * FOP-WU3-VERIFY-R1 — offline verification harness for
...
const HELPER_SHA256_EXPECTED = 'f3fc4d0cb40ef60bfb97ac05df0623793fe52badeb2db0a07ca83911d7a2453e';
...
const ARTIFACTS = path.join(HERE, 'fop-wu3-artifacts');
...
for (const pair of [ID9, ID10]) {
  ...
  report(
    `CASE-A-${pair.id}-preflight-ok-inprocess`,
    ok,
    'n/a',
...
report(
  ...
  "helper's mismatch branch (helper line 1076: `identity.ref !== args['expect-target-ref']`) evaluates false; " +
...
const scanPaths = [SELF_PATH, HELPER_PATH, ...fs.readdirSync(ARTIFACTS).map((f) => path.join(ARTIFACTS, f))]
...
report(
  ...
  `swarmctl secret-scan over ${scanPaths.length} paths -> exit ${scan.status}, findings=${Array.isArray(scanFindings) ? scanFindings.length : 'UNPARSED'}: ${oneLine(scanOut, 400)}`,
);
...
if (failures.length === 0) {
  console.log(`FOP_HELPER_VERIFY_TOTAL: ${passed}/${caseCount} PASS`);
  process.exitCode = 0;
} else {
  console.log(`FOP_HELPER_VERIFY_TOTAL: ${failures.length} FAILURE(S)`);

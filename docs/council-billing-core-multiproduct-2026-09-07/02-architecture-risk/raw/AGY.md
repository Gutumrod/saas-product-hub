# AGY — Independent Expert Report — WSTERA Billing Core Multi-Product Profile Architecture
...
| Billing Core Plan | `docs/platform/BILLING_CORE_PLAN.md` (v4, LOCKED 2026-08-27) | Centralized `billing-core` service; thin host-level orchestrator; Stripe card = recurring rail; PromptPay = manual/non-auto-renew; reconciliation mandatory before PromptPay; PawSpace narrow ingress; transactional outbox webhook; RPO ≤ 1h; test-mode-only until go-live; `sk_live_` boot guard; two-billing-systems accepted consequence (BK01 excluded) |
| Phase 0.5 Security Contracts | `docs/platform/BILLING_CORE_PHASE_0_5_SECURITY_CONTRACTS.md` (rev 2, 2026-08-30) | Threat model T1–T20; `/v1/*` auth + account-ownership contract; product-wide account reach (not account isolation) without assertion; transactional outbox webhook intake; `billing_core` schema access model; vendor pin `3b6401a`; `scheduled()` contract; R15 pre-data gate |
...
| Product Build-to-Sell briefs | `products/{booking,DocCraft,PawSpace,WSTERA-Link,WSM,multi-tenant-ai,booking-ticket-module}/docs/BUILD-TO-SELL-EXECUTION-2026-09-06.md` | Current commercial-mode reality per product (see §2.3) |
...
### 2.3 Current commercial-mode reality (from product briefs — newer Owner authority)
...
- **MT01:** one-time source product, USD 149–199 single purchase, perpetual use of purchased version + 12 months updates.
...
```
WSTERA Products
   │  (per-product caller credential, request-scoped)
   ▼
Central Billing API (billing-core Worker, Hono)
   │  request-scoped canonical product_id + account/tenant identity + environment
   ▼
Product Billing Profile Registry  (billing_core schema, versioned profiles)
   │  resolve profile by (product_id, environment, profile_version)
   ▼
...
Versioned profile schema (JSON document in `billing_core` schema, or normalized columns — recommend a versioned JSONB `profile` column plus a `profile_version` integer and immutable history rows). Must represent at least:
...
**Rules:** profile is immutable once `active`; changes create a new `profile_version` with an explicit migration/rollback rule; runtime must never silently pick a partially-written profile (activation only after all required fields + mappings + tests complete — see §3.13).
...
| `GET /v1/subscriptions/:product/:accountId` | caller credential + product match | `:product` must equal credential's product; cross-product → 403; product-wide read unless account-bound assertion | Serves LK01/DC01 pull model; PawSpace reads its own DB |
...
| `POST /v1/portal` | caller credential + product match + **account-bound assertion REQUIRED** | valid Stripe Billing Portal session exposes payment-method/cancel/invoice/subscription actions — bearer credential alone insufficient | Short-lived; server-selected allowlisted return URL; rate-limited per credential AND per account |
...
**Every request enforces:** server-side product identity (from credential), account/tenant identity, environment, authorization, idempotency, valid profile/plan. **Prevents:** wrong product, cross-product, cross-account, caller product spoofing, caller-supplied Stripe Price manipulation (price comes from server-side profile mapping, never caller). **Return URLs** resolve from server-side allowlists only (T11). **Account isolation** claims are backed by account-bound assertions (§2.3 of Phase 0.5); product credential alone is product-granularity authority only.
...
- **Where IDs are stored:** in the Product Billing Profile Registry (`billing_core` schema), per environment (`test`/`live`), as **config/mapping data** — NOT domain/commercial truth.
...
- **Price change/version migration:** profile version bump + new immutable Stripe Price + admission re-test; existing subscriptions continue on their current Price until renewal/change per approved financial policy (proration policy is externalized to the CEO's financial plan — not chosen here).
...
- **Metadata convention:** `metadata.product` + `metadata.account_id` are **routing hints only** — never authority for money truth or entitlement truth. An event with no recognized `metadata.product` is audit-logged and answered `200`, never guessed at.
...
- Unknown/absent `product` metadata → audit-logged `skipped`, `200`, no guess (canonical plan §2b).
...
Locked contract (from Phase 0.5 §4):
1. Read raw body; 64KB cap → 413 if over.
2. Verify `Stripe-Signature` → 401 if invalid.
3. Parse; missing `event.id` → T10 `skipped` (alerted audit row, no state), 200.
4. Read `metadata.product`/`metadata.account_id`; unknown product → `skipped`, 200, no guess.
5. **One transaction:** insert `processed_events` (UNIQUE on stripe_event_id) + `delivery_jobs` outbox row; commit.
6. Duplicate → read status: completed/skipped → `200 {duplicate:true, completed:true}`; pending/processing/retryable-failed → ensure due job, `200 {duplicate:true, completed:false}`; cannot prove durable retry → non-2xx.
...
- **PS01 → Pawstia:** authoritative subscription lifecycle via narrow signed transition ingress (existing `transition_shop_subscription`/`set_shop_commercial_package` RPCs).
...
- Use Stripe Test Mode on the one WSTERA Stripe account as **`WSTERA Stripe Sandbox / Billing Test Fixture`** (NOT "Queueeasy Stripe Fixture" — Queueeasy is a separate shared LINE OA test fixture).
...
- **Explicit proof Product A cannot modify Product B entitlement** (isolation test — mandatory admission gate).
...
- Staging: `billing_core_staging` schema + `billing_core_staging_app` role in Project A (no new project, no Pro/branch DB per §10 D10).
...
1. **REGISTER PROFILE** (draft) — canonical product_id from `registry.yaml`, billing models, currency, plans, rails, lifecycle refs, entitlement adapter ref, refund policy ref.
2. **AUTHORIZATION BINDINGS** — per-product caller credential issued, environment- and audience-bound; account-bound assertion mechanism defined for portal/account routes.
3. **CREATE/MAP STRIPE PRODUCT** (test) — one Stripe Product per WSTERA Product.
4. **CREATE/MAP PRICE** (test) — immutable Stripe Price per plan/version.
5. **DEFINE ENTITLEMENT ADAPTER** — signed ingress or snapshot contract; replay/idempotency; TTL.
6. **RECONCILIATION CAPABILITY** — product's expected-payment matching (amount/currency/product/account/plan/billing period) wired into shared reconciler.
7. **RUN TEST SUITE** — unit/integration/webhook/reconciliation/entitlement/negative/failure.
8. **PASS ISOLATION** — explicit proof Product A cannot modify Product B entitlement; cross-product/cross-account negatives.
9. **AUDIT CONTROLS** — redacted audit projection, correlation, retention.
10. **ACTIVATE** — only after ALL of the above complete. Activation must be impossible if any required environment/provider mapping, authorization binding, reconciliation capability, entitlement contract, test, or audit control is incomplete.
...
- **Later migration** (after central proof on PS01/LK01/DC01) is a separate, evidence-gated decision — not authorized now.
...
- **Recorded consequence:** the portfolio runs two billing systems during transition (BK01 inline + billing-core). This is intentional and already accepted in the canonical plan (§5d). Any provider-integration defect must be triaged against both.
...
| 6 | Webhook out-of-order | provider_state_updated_at comparison | monotonic transition rules; reconcile before ambiguous regression; stale → `skipped` | reconciler re-fetch | no regression | `skipped` audit row | billing-core ops |
...
| 9 | Wrong Product | metadata.product + credential binding | unknown/absent product → `skipped`, 200, no guess; cross-product → 403 | none | no grant | `skipped`/403 audit | billing-core ops |
...
| 19 | PromptPay expiry | reconciler / expiry check | no grant; notify; per policy (grace→downgrade or end paid) | reconciler | no grant | alert | billing-core ops |
...
1. **BK01 central facade vs. later migration timing** — the compatibility path is recommended (facade now, migration later after central proof), but the *timing* of any eventual BK01 migration is an Owner decision, not an architecture-lock blocker.
2. **Reconciliation cadence per product** — default daily (Payment Council) / 15-min cron (canonical plan); per-product override is config, but the default cadence for the first product is an Owner/ops decision.
3. **Refund/cancel/renewal financial policy** — externalized to the CEO's financial plan (canonical plan §5d); not chosen here.
4. **Product #1 admission order** — PS01 is the natural first (narrow ingress + Phase 13 RPCs ready), but the Owner confirms which product is admitted first.
...
The Phase 0.5 security contracts (T1–T20) are the strongest part of the existing evidence and directly satisfy the brief's security invariants. Key confirmations:

- **Product credential binding alone is NOT same-product account isolation** (T16, §2.3). Any stronger account-bound claim requires an account-bound assertion. This is already the canonical position and the brief's source-drift note confirms it. The billing API contract (§3.5) enforces this: `/v1/portal` requires the assertion; account-reading GET routes are product-wide unless asserted.
...
**Residual risks (accepted, not blockers):** a Project-A outage takes billing offline (lost sale, not data loss — CEO-accepted); a billing-core Worker compromise yields test Stripe key + billing schema + bounded PawSpace transitions (not tenant data); the two-billing-systems transition (BK01 + billing-core) requires dual triage.

---

## 5. Failure-case analysis

All 20 required cases are covered in the matrix (§3.16). The design does not solve integrity problems by trusting redirects, webhook delivery order, metadata alone, or mutable in-memory profile state. The two most dangerous cases — **webhook out-of-order** (T20) and **entitlement-without-valid-payment** (case 15) — are both handled by the reconciler re-fetching provider truth and applying monotonic transition rules, not by trusting delivery order or metadata.

---

## 6. BK01 compatibility decision

See §3.14. **Transitional exception + central facade/adapter, later migration after central proof.** No forced migration; no merge of BK01 merchant/customer PromptPay deposit into WSTERA SaaS billing.

---

## 7. Profile/schema/API/webhook/reconciliation/entitlement/test/admission contracts

All defined in §3.3–§3.13. These are build-usable and consistent with the frozen evidence.

---

## 8. Rejected alternatives and why

1. **Per-product independent Stripe integration (status quo for non-BK01):** rejected — multiplies money-correctness defect surface; the canonical plan already found two real shared-module bugs a passing suite missed. Centralization is a frozen Owner decision (#1).
2. **Mutable global `current_profile`/`active_product`:** rejected — cross-route race by construction; violates the brief's critical invariant.
3. **PromptPay as a second subscription state machine:** rejected — frozen decision (#5) + current Stripe provider behavior (one-time, user-initiated, THB-only, not auto-recurring).
4. **Forced BK01 migration for architecture symmetry:** rejected — frozen decision (#9); would destabilize a mature, working integration.
5. **Billing-as-a-Service productization now:** rejected — non-goal; internal-first then separately gated BaaS option (Payment Council).
6. **Trusting webhook delivery order / metadata / redirect as money truth:** rejected — integrity must come from provider re-fetch + reconciliation + idempotency ledger.
7. **A second Supabase project / Pro / branch DB for billing:** rejected — Free-First policy + §10 D10 (no new project, no Pro until revenue funds it); schema isolation in Project A instead.

---

## 9. Assumptions

1. **Stripe account is Thailand-based / PromptPay-eligible** — the canonical plan's Phase 0 preflight must verify this (P-2). Not yet verified as fact; it is a preflight gate.
2. **Stripe API version is pinned** before adapter/E2E evidence is accepted (P-2). Assumed to be done in Phase 0.
3. **The `billing_core_staging` schema + preview Worker** will be stood up in Phase 0 (P-1 condition 4). It does not exist yet — this is a Phase 0 scope item, not an assumption to build against.
4. **Pawstia completes its PS-A2 schema-scoping + Project B admission** before the ingress implementation gate. Currently blocked on that (Phase 0.5 §7.2).
5. **`modules-hub` pin `3b6401a`** remains the vendored source (Phase 0.5 §5). Assumed stable.
...
1. **Stripe Thailand account PromptPay eligibility** — not yet verified; Phase 0 preflight gate.
2. **`billing_core_staging` environment** — does not exist yet; Phase 0 scope.
3. **Pawstia Project B admission** — pending PS-A2 evidence + explicit authorization; blocks ingress implementation.
4. **hub-web R15 pre-data gate** — hub-web runtime still on Project A `postgres` owner URL; must move to scoped `hub_web_app` role before any billing data exists.
5. **Reconciliation cadence default** for the first product — Owner/ops decision.
6. **Refund/cancel/renewal financial policy** — externalized to CEO's financial plan; not yet locked here.
...
**PASS** (with the locked-contract conditions in §3 and the implementation-phase gates in §10).

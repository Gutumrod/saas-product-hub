# Independent Expert EXPERT REPORT â€” WSTERA Billing Core Multi-Product Profile Architecture

**Expert:** Independent Expert (independent clean-room)
...
**ADOPT the Owner candidate** â€” `WSTERA Products -> Central Billing API -> request-scoped canonical product_id -> Product Billing Profile Registry -> Provider Adapter -> Stripe` â€” **with the hardening conditions below**, and propose Council verdict **REMEDIATE** (not PASS, not BLOCK).

Rationale in one line: the centralized-billing direction is already LOCKED and evidence-backed (BILLING_CORE_PLAN.md v4, Phase 0.5 security contracts, 2026-09-01 Payment Council synthesis), and the multi-product profile registry is a sound, buildable extension of that locked direction â€” but the frozen brief asks for a *complete, build-usable* contract across 21 output areas, and several of those contracts are not yet fully specified or are contradicted by current evidence. A PASS would overstate lock-readiness; a BLOCK is not warranted because every gap is a bounded, nameable remediation that does not require re-litigating the frozen Owner decisions.
...
- `docs/platform/BILLING_CORE_PHASE_0_5_SECURITY_CONTRACTS.md` (rev 2, 2026-08-30): threat model T1â€“T20; `/v1/*` auth contract; **product-wide account reach** (product credential is product-granularity only, NOT account isolation â€” T16, Â§2.3); account-bound assertion required for `/v1/portal`; transactional outbox webhook intake (Â§4); vendor provenance/pin policy (Â§5, pin = `modules-hub` main `3b6401a28e6f7e69b383277a200fca1986f49ede`); Worker `scheduled()` contract (Â§6); R15 pre-data gate (hub-web must move off the Project A `postgres` owner `DATABASE_URL` before any billing data exists).
- The brief's "product credential binding alone is not same-product account isolation" is **explicitly confirmed** by Phase 0.5 Â§2.3 and T16.

### 1.5 Current commercial-mode reality (newer Owner authority) â€” verified from the 7 product Build-to-Sell briefs
...
- **DC01 DocCraft** (`products/doccraft/docs/...`): Free Public Pilot first; later one-time unlock experiment THB 299â€“599; **subscription is NOT V1**.
- **MT01** (`products/multi-tenant-ai/BUILD-TO-SELL-EXECUTION-2026-09-06.md`): one-time source product, USD 149â€“199 single purchase, perpetual use of purchased version + 12 months updates.
- **CM01** (`products/booking-ticket-module/docs/...`): one-time commercial source/license product; final pricing unresolved.

### 1.6 Drift the brief flags â€” confirmed
- `BILLING_CORE_PLAN.md` still carries stale DocCraft subscription language; current DC01 authority is free Public Pilot then a later one-time unlock experiment. **Confirmed** â€” the plan's Context section lists `doccraft` under the "Subscribe" group, which contradicts the current DC01 brief (subscription is not V1). This is document drift, not a re-litigation.
- The plan historically accepted two billing systems because BK01 was excluded. **Confirmed** â€” Â§5d: "the portfolio will run two billing systems permanently â€” booking's inline integration and billing-core. This is intentional." This run evaluates compatibility/facade convergence without forced migration.
- Phase 0.5 webhook contract is a transactional outbox/durable delivery, not a presence-only event ledger. **Confirmed** â€” Â§4.
...
- **Price immutability:** Stripe Price objects are immutable â€” `unit_amount` cannot be updated after creation; to change price you create a new Price under the same Product and migrate subscriptions. (docs.stripe.com/api/prices/object; confirmed by multiple sources)
- **Idempotency:** Stripe idempotency keys are up to 255 chars; results cached for â‰¥24h; reusing a key with different parameters returns an `idempotency_error` (409); keys expire after 24h. (docs.stripe.com/api/idempotent_requests)
...
- One server-side billing entry path reduces the "four places failing quietly" problem the owner cited as the reason to centralize (BILLING_CORE_PLAN.md Â§5b).
...
- Do NOT force all Products into the same monetization model â€” the registry must represent `free`, `trial`, `subscription`, `one_time`, `manual_renewal` and let each Product use the rails appropriate to it.
...
```
- **Profile mutation/version changes MUST have an explicit migration/rollback rule** (Frozen brief Â§New Product admission). Runtime must not silently pick a partially-written profile â€” reads must be atomic against a fully-committed version.
...
- `POST /v1/checkout` â€” create/reuse Stripe Customer, create subscription-mode Checkout Session. **Server-derived idempotency key** (environment + credential/product + account + operation + immutable logical checkout-attempt ID), never caller-supplied (T13). DB lock/reservation before Stripe Customer call so concurrent retry cannot create a duplicate Customer. Return URL from server-side allowlist only (T11). Account-bound assertion recommended.
- `GET /v1/subscriptions/:product/:accountId` â€” caller credential + product match; `:product` must equal credential's product; cross-product â†’ 403. Without account-bound assertion this is **product-wide read** (explicit acceptance per product).
- `GET /v1/entitlements/:product/:accountId/:key` â€” same product-wide-read caveat.
- `POST /v1/portal` â€” **account-bound assertion REQUIRED** (T16/Â§2.2); short-lived session; server-selected allowlisted return URL; rate-limited per credential AND per account.
...
- **Metadata convention:** `metadata.product`, `metadata.account_id`, `metadata.profile_version`, `metadata.environment` are **routing hints only** â€” they may route a webhook to the right product/account but must never by themselves authorize money truth or entitlement truth (Frozen brief Â§Webhook routing).
...
- **Duplicate/out-of-order:** `processed_events` UNIQUE on Stripe event ID + explicit status; duplicate â†’ `200 {duplicate:true}`; out-of-order distinct events â†’ compare provider object/version timestamp with current snapshot, monotonic transition rules, reconcile before ambiguous regression, record stale event as `skipped` (T20).
...
- **Signed ingress:** narrow, signed, timestamped, replay-bounded (e.g. PawSpace Project B Edge Function ingress; Phase 0.5 Â§1.1/Â§4).
...
- Examples (from brief): PS01 â†’ Pawstia authoritative subscription lifecycle via narrow signed transition ingress; LK01 â†’ bounded local entitlement snapshot off redirect hot path; DC01 â†’ future one-time unlock/license entitlement preserving local-first; source-sale products (MT01/CM01) â†’ purchase/fulfillment/license/update entitlement, NOT a fake recurring subscription.

### 2.11 Test Fixture Contract
- Use Stripe Test Mode on the one WSTERA Stripe account as **`WSTERA Stripe Sandbox / Billing Test Fixture`** (NOT "Queueeasy Stripe Fixture" â€” Queueeasy is a separate Shared LINE OA test fixture, per BUILD-TO-SELL-EXECUTION-2026-09-06.md).
...
- Staging: `billing_core_staging` schema + scoped role in Project A (no new project, no Pro/branch DB â€” Â§10 D10, Free-First).
...
- The portfolio will run two billing systems during transition (already accepted in BILLING_CORE_PLAN.md Â§5d); the facade converges the product-facing entry contract over time.
...
| 6 | Webhook out-of-order | provider timestamp vs snapshot | Monotonic rules; reconcile before ambiguous regression; stale event â†’ `skipped` | Reconciler | No regression of newer state | `skipped` audit row | billing-core |
...
| 9 | Wrong Product | metadata routing + provider re-fetch | Event with unrecognized product â†’ audit-logged, `200`, no guess | N/A | No cross-product grant | `skipped` audit row | billing-core |
...
The Phase 0.5 security contracts (T1â€“T20) are the strongest part of the evidence and are directly reusable. Key invariants that MUST hold in the multi-product profile design:

1. **No mutable global active/current profile** â€” the load-bearing rule. Every request/event/job resolves product/profile request-scoped from canonical identity + environment. Concurrent Product A and Product B operations must be impossible to cross-route.
2. **Product credential is product-granularity only, not account isolation** (T16/Â§2.3). Any stronger account-bound claim requires an account-bound assertion or equivalent verified ownership proof. `/v1/portal` never runs without the assertion.
3. **No caller-supplied provider price authority** â€” Price resolved server-side from profile `provider_mappings` by `plan_id`.
4. **Environment separation** â€” test/live mapping isolation; live keys impossible before Owner authorization + boot guard.
5. **Durable webhook intake/outbox** â€” `processed_events` UNIQUE + `delivery_jobs` outbox in one transaction; `200` only after durable intake AND durable delivery obligation.
6. **Provider re-fetch + amount/currency validation** â€” webhook/redirect body alone is never authoritative money truth.
7. **No raw sensitive payload/secret logging** â€” redacted allowlisted audit projection.
8. **R15 pre-data gate** â€” hub-web must move off the Project A `postgres` owner `DATABASE_URL` before any billing data exists.
...
Covered in full in Â§2.16 (all 20 required cases). Summary of the highest-risk cases and their non-negotiable controls:
- **#14 payment succeeds but entitlement update fails** â€” must NOT be solved by trusting the redirect; the paid state is recorded durably and entitlement is retried via the outbox + reconciler.
- **#15 entitlement exists without valid payment** â€” reconciler must detect and revoke; entitlement alone is never authority.
- **#6 out-of-order** â€” monotonic transition rules + reconcile-before-regression; never let an older distinct event overwrite newer state.
- **#20 reconciliation drift** â€” re-fetch provider truth, idempotent correction, operator review for ambiguous cases.
- **#18 card retry failure** â€” `past_due` blocks entitlements immediately (Phase 0 fix); grace only while `gracePeriodEnd` hasn't passed (fail closed if missing/expired).
...
All specified in Â§2.3â€“Â§2.13. The contracts are build-usable with the hardening conditions in Â§8.
...
- The `modules-hub` pin `3b6401a28e6f7e69b383277a200fca1986f49ede` remains the vendor source (Phase 0.5 Â§5).
...
1. **Define the Product Billing Profile Registry** as a first-class versioned object (schema in Â§2.3, lifecycle in Â§2.4) with an explicit migration/rollback rule and a fail-closed admission gate (Â§2.13). This is the single most important missing contract.
2. **Record Stripe Test preflight evidence** (Thailand account PromptPay eligibility, PromptPay in test mode, pinned API version) â€” Phase 0 prerequisite.
